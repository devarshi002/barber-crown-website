...
app.get('/api/availability', async (req, res) => {
  ...
  const allSlots = ['9:00 AM','10:00 AM','11:00 AM','12:00 PM','1:00 PM','2:00 PM','3:00 PM','4:00 PM','5:00 PM','6:00 PM','7:00 PM'];
  const booked = await db.collection('bookings').find({ date, status: { $ne: 'cancelled' } }).toArray();

  // Count bookings per slot
  const slotCounts = {};
  allSlots.forEach(s => slotCounts[s] = 0);
  booked.forEach(b => { if (slotCounts[b.time] !== undefined) slotCounts[b.time]++; });

  const available = allSlots.filter(s => slotCounts[s] < 3);
  res.json({ date, available, slotCounts, booked: booked.map(b => b.time) });
});
